...
### [2025-11-06] - אופיר ברנס - שיפור ממשק ניהול API Keys לכל משתמש בנפרד

**מה שונה:**
- עדכון `src/components/AdminDashboard.tsx` - הוספת ממשק נוח להגדרת API key לכל משתמש בנפרד בטאב Settings
- עדכון `src/components/StoryCreator.tsx` - שימוש ב-API key של המשתמש במקום הגלובאלי
- עדכון `src/components/WorkbookCreator.tsx` - שימוש ב-API key של המשתמש במקום הגלובאלי (בשני המקומות)
- הוספת dropdown לבחירת API key לכל משתמש בנפרד
- הצגת סטטוס מפתח נוכחי לכל משתמש
- Fallback למפתח גלובאלי אם משתמש לא קיבל מפתח

**למה:**
- ממשק נוח יותר להגדרת API key לכל משתמש בנפרד
- כל משתמש ישתמש במפתח שלו ולא במפתח הגלובאלי
- ניהול טוב יותר של מפתחות API - כל משתמש יכול לקבל מפתח משלו
- בדיוק כמו שהוא עובד עכשיו - ללא תקלות

**השפעה:**
- ✅ ממשק נוח וברור להגדרת API key לכל משתמש בנפרד
