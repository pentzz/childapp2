...
1. [מידע כללי על הפרויקט](#מידע-כללי-על-הפרויקט)
2. [משיכת הפרויקט מה-Git של השרת](#משיכת-הפרויקט-מה-git-של-השרת)
3. [מבנה הפרויקט](#מבנה-הפרויקט)
4. [משתני סביבה](#משתני-סביבה)
5. [תהליך Deploy](#תהליך-deploy)
6. [אזהרות חשובות](#אזהרות-חשובות)
7. [פקודות חשובות](#פקודות-חשובות)
8. [ארכיטקטורה ומבנה](#ארכיטקטורה-ומבנה)
...
1. **אל תמחק את `AppContext.tsx`** - זה יפיל את כל האפליקציה
2. **אל תשנה את `vite.config.ts` בלי הבנה** - זה ישבור את ה-Build
3. **אל תמחק את `index.html`** - זה ה-entry point
4. **אל תעלה `.env.local` ל-Git** - זה מכיל סודות!
5. **אל תשנה את `deploy-vite-project.sh` בלי לבדוק** - זה ישבור את ה-Deploy
6. **אל תמחק את `/public/logo.png`** - זה הלוגו
7. **אל תשנה את מבנה ה-`src/components/` בלי לבדוק** - זה ישבור imports

### **✅ תמיד תעשה:**

1. **עשה `git pull` לפני שינויים** - תמיד להיות מעודכן
2. **בדוק את ה-Console** - אחרי שינויים, בדוק שגיאות
3. **עשה `npm install`** - אחרי `git pull` אם יש שינויים ב-`package.json`
4. **עשה `npm run build` מקומית** - לפני Push לשרת
5. **בדוק את ה-TypeScript** - `npm run build` יראה שגיאות
6. **עשה Commit עם הודעות ברורות** - זה עוזר ל-Troubleshooting
